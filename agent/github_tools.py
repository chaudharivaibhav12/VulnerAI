...
# ─────────────────────────────────────────────────────────────
# VULN-* (web-app vuln) PR — uses richer catalog + ranking context
# ─────────────────────────────────────────────────────────────

def create_vuln_pull_request(
    branch_name: str,
    vuln_id: str,
    catalog_row: dict,
    ranking_entry: Optional[dict],
    patch_files: list[str],
) -> dict:
    """
    Open a PR for a web-app vulnerability (VULN-*).

    The body is built from the vuln_catalog.ndjson row plus, when available,
    the Ranking Agent's ranking entry (composite score, evidence, sample
    payload). Falls back gracefully when ranking_entry is None.
    """
    name        = catalog_row.get("name", vuln_id)
    cwe         = catalog_row.get("cwe", "")
    cvss        = catalog_row.get("cvss_score", 0.0)
    description = catalog_row.get("description", "")
    endpoint    = catalog_row.get("endpoint", "")
    method      = catalog_row.get("http_method", "")
    param       = catalog_row.get("vulnerable_param", "")
    sink        = catalog_row.get("sink_function", "")
    file_path   = catalog_row.get("file_path", "")
    line_no     = catalog_row.get("line_number", "?")
    fix_strat   = catalog_row.get("fix_strategy", "")
    severity    = catalog_row.get("static_severity", "")

    # Optional ranking-agent evidence
    rank        = (ranking_entry or {}).get("rank", "?")
    composite   = (ranking_entry or {}).get("composite_score")
    evidence    = (ranking_entry or {}).get("evidence") or {}
    sub_scores  = (ranking_entry or {}).get("sub_scores") or {}
    sample_tid  = (ranking_entry or {}).get("sample_trace_id", "")
    sample_pl   = (ranking_entry or {}).get("sample_payload", "")
    reasoning   = (ranking_entry or {}).get("reasoning", "")

    attack_count = evidence.get("attack_count_1h", 0)
    unique_asns  = evidence.get("unique_asns", 0)
    countries    = evidence.get("countries") or []
    kev_listed   = evidence.get("kev_listed", False)
    wild_hint    = (evidence.get("live_signals") or {}).get("wild_hint", False)

    composite_md = f"{composite:.2f}" if isinstance(composite, (int, float)) else "n/a"
    files_md     = "\n".join(f"- `{f}`" for f in patch_files) or "- (none)"
    timestamp    = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")

    pr_body = f"""\
## 🛡️ VulnerAI Patcher Agent — Fix for `{vuln_id}`

> **Autonomous PR** opened by the [VulnerAI Patcher Agent](https://github.com/{Config.GITHUB_REPO}) \
after the Ranking Agent identified this vulnerability as the top remediation candidate. \
Generated at {timestamp}.
...
| Field | Value |
|---|---|
| **Vuln ID** | `{vuln_id}` — {name} |
| **CWE** | `{cwe}` |
| **CVSS** | `{cvss}` ({severity}) |
| **Ranking** | #{rank}  ·  composite score **{composite_md}** |
...
### 📡 Runtime Evidence (read_apm)

| Signal | Value |
|---|---|
| **Attack attempts (1h)** | {attack_count} |
| **Unique source ASNs** | {unique_asns} |
| **Source countries** | {", ".join(countries) if countries else "—"} |
| **Sample trace_id** | `{sample_tid or "—"}` |
| **Sample payload** | `{sample_pl or "—"}` |

### 🌐 External Threat Pressure (search_nimble)

| Signal | Value |
|---|---|
| **CISA KEV-listed** | {"🔴 YES" if kev_listed else "🟢 No"} |
| **MITRE confirms 'in the wild'** | {"🔴 YES" if wild_hint else "🟢 No"} |

### 🧮 Composite sub-scores

| Component | Score (0–100) |
|---|---|
| ACTIVE_EXPLOITATION | {sub_scores.get('active_exploitation', 0):.1f} |
| EXTERNAL_PRESSURE   | {sub_scores.get('external_pressure',   0):.1f} |
| STATIC_SEVERITY     | {sub_scores.get('static_severity',     0):.1f} |

> **Why this ranked #{rank}:** {reasoning or "—"}

---

### 🔧 Fix Strategy

{fix_strat or "(none on catalog)"}

The diff in this PR substitutes the vulnerable source with the hardened reference
implementation from the catalog's `reference_patch_file`. See the **Files changed**
tab for the exact change.
...
*Generated by [VulnerAI](https://github.com/{Config.GITHUB_REPO}) — \
Claude Sonnet 4.6 · ClickHouse · Nimble · Datadog APM*
"""

    title = f"[VulnerAI] {vuln_id}: {name} ({cwe}, CVSS {cvss})"

    if _is_dry():
        mock_pr_number = abs(hash(branch_name)) % 9000 + 1000
        mock_url = f"https://github.com/{Config.GITHUB_REPO}/pull/{mock_pr_number}"
        print(f"[github] [DRY RUN] Would create PR: {title}")
        print(f"[github] [DRY RUN] Branch: {branch_name} → {Config.GITHUB_BASE_BRANCH}")
        print(f"[github] [DRY RUN] PR URL would be: {mock_url}")
